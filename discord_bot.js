...
var commands = {
	...
	"trump": {
		description: "Shows you pictures of Donald Trump",
		process: function(bot,msg,suffix){
			var images = [
				"https://timedotcom.files.wordpress.com/2015/01/trump.jpg",
				"https://martinomalley.com/wp-content/uploads/2015/09/dt_gun_pic.jpg",
				"https://localtvkstu.files.wordpress.com/2016/02/trump1.jpg",
				"http://www.investors.com/wp-content/uploads/2016/11/POLL-Trump-20-AP.jpg",
				"http://www.wfuv.org/sites/default/files/trump-announce_wide-1143fdac01ac69c46c23467357c57626a699948f_0.jpg"
			]
	...
	"8ball": {
		...
		process: function(bot,msg,suffix){
			var sayings = [ "It is certain", //8ball results
                "It is decidedly so",
                "Without a doubt",
                "Yes, definitely",
                "You may rely on it",
	...
	"verify": {
		usage: "<username>",
		description: "Verifys the user.",
		process: function(bot,msg,suffix){
			var role = roles.get(msg.author, "Member").id;
			var rolecheck = msg.guild.roles.find("name","Member");
			if(!rolecheck){
				return msg.reply(":x: This channel does not have the 'Member' role created!");
			}
			if(bot.memberHasRole(msg.author,rolecheck)){
				return msg.reply(":x: You have already verified yourself!");
			} else {
				bot.addMemberToRole(msg.author,rolecheck);
				msg.reply(":white_check_mark: You have successfully been verified!");
			}
		}
	},
	"msg": {
	...
	if(msg.author.id != bot.user.id && (msg.content.startsWith(Config.commandPrefix))){
        ...
        if(cmdTxt === "help"){
            //help is special since it iterates over the other commands
						if(suffix){
							...
							msg.author.sendMessage("**Available Commands:**").then(function(){
								...
								msg.author.sendMessage("__**Cyrus is a multi-purpose bot created by <@>**__\nFound a bug with cyrus? Post it in our official discord channel!\nhttp://discord.gg/m9SqURN\n\n Want me in your server? https://discordapp.com/oauth2/authorize?&client_id=269808956883468289&scope=bot&permissions=470019135");
						});
					}
        }
		else if(cmd) {
			if(Permissions.checkPermission(msg.author,cmdTxt)){
				...
				msg.channel.sendMessage(":x: You are not allowed to run " + cmdTxt + "!");
			}
		} else {
