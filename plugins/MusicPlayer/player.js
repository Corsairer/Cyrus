...
let options = false;
	let PREFIX = (options && options.prefix) || '_';
...
exports.play = {
		...
		description: "Plays the given video in the user's voice channel. Supports YouTube and many others",
...
exports.queue = {
	 ...
	 */
exports.uneueqe = {
	description: "Unqueues the given song index from the song queue.  Use the queue command to get the list of songs in the queue.",
	process: function(client, msg, suffix) {
		// Define a usage string to print out on errors
		const usageString = 'The format is "-unqueue <index>".  Use -queue to find the indices of each song in the queue.';
